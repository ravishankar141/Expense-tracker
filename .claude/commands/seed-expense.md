...
Read database/[db.py](http://db.py) to understand the expenses table schema, the database connection pattern, and the database
file name.
...
- user_id integer
- count integer, the number of expenses to create

- months integer, how many past months to spread them across

If any argument is missing or is not a valid integer, stop and say:
...
Before generating anything, confirm the user id exists in the users table. If not, stop and say:
...
5. Uses parameterised queries only string formatting in SQL no
